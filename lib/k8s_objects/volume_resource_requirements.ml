(* Anvil source: kubernetes_api_objects/spec/volume_resource_requirements.rs
   [VolumeResourceRequirementsView]. Structurally identical to
   [Resource_requirements] but a DISTINCT type (the PVC-side requirements);
   referenced by PersistentVolumeClaimSpecView.resources. *)
