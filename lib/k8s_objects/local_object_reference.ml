(* Anvil source: kubernetes_api_objects/spec/pod.rs [LocalObjectReferenceView].
   A ghost placeholder struct `{}`; ported as the unit/empty type. Referenced by
   PodSpecView.image_pull_secrets as Seq<LocalObjectReferenceView>. *)
...
let of_json (j : Yojson.Safe.t) : t Res.t =
  ...
  | `Null | `Bool _ | `Int _ | `Intlit _ | `Float _ | `String _ | `List _ ->
    Res.error
      (Err.Decode_error
         { typ = "LocalObjectReference"; detail = "expected an object" })
